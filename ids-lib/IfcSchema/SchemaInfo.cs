...
namespace IdsLib.IfcSchema
{
	...
	public partial class SchemaInfo : IEnumerable<ClassInfo>
	{
		...
		{
			var occurrenceClass = this[objClass];
			if (occurrenceClass is null)
			{
			...
			{
				var typeClassInfo = this[typeClass];
				if (typeClassInfo != null)
				{
					typeClassInfo.FunctionalType = FunctionalType.TypeOfElement;
					if(typeClassInfo.Type == ClassType.Concrete)
					{
						found.Add(typeClass);
					}

					var concreteTypes = typeClassInfo.SubClasses.Where(c => c.Type == ClassType.Concrete).Select(c => c.Name.ToUpperInvariant());
					
					found.AddRange(concreteTypes);
				}
			}
			if (found.Any())
			{
				...
				occurrenceClass.AddTypeClasses(found);
			}
