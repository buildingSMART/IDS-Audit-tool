# IfcClassInformation class

Metadata container for entities of an IfcSchema

```csharp
public class IfcClassInformation
```

## Public Members

| name | description |
| --- | --- |
| [IfcClassInformation](IfcClassInformation/IfcClassInformation.md)(…) | Default constructor, ensures static nullable analysis |
| [PascalCaseName](IfcClassInformation/PascalCaseName.md) { get; } | Name of the attribute as a string, stored in PascalCase |
| [UpperCaseName](IfcClassInformation/UpperCaseName.md) { get; } | Name of the attribute as a string, converted to UPPERCASE |
| [ValidSchemaVersions](IfcClassInformation/ValidSchemaVersions.md) { get; } | Versions of the schema that contain the class |

## See Also

* namespace [IdsLib.IfcSchema](../ids-lib.md)
* [IfcClassInformation.cs](https://github.com/buildingSMART/IDS-Audit-tool/tree/main/ids-lib/IfcSchema/IfcClassInformation.cs)

<!-- DO NOT EDIT: generated by xmldocmd for ids-lib.dll -->
